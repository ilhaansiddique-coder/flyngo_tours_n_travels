...
async function main() {
    ...
    console.log(`   demo content: ${SEED_DEMO_CONTENT ? 'ENABLED' : 'disabled (set SEED_DEMO_CONTENT=true to enable)'}`);
    // ===========================================================================
    // 1. DEFAULT TENANT
    // ===========================================================================
    ...
    if (!adminPlainPassword && !existingAdmin) {
        throw new Error('SUPER_ADMIN_PASSWORD must be set when seeding a fresh database with NODE_ENV=production. ' +
    ...
    const visaCountries = [
        {
            ...
            order: 1,
            content: {
                intro: 'Dubai has become one of the most popular destinations for Bangladeshi travellers, whether for tourism, transit or long-term residence. This comprehensive guide covers all the visa options, requirements, fees and processing times so you can apply with confidence.',
                pricingTiers: [
                    {
                        id: 'tourist-30',
        ...
        },
        {
            ...
            order: 2,
            content: {
                intro: 'Malaysia is a favourite of Bangladeshi travellers for its affordability, culture and food. We process e-Visa and eNTRI applications quickly and transparently.',
                pricingTiers: [
                    {
                        id: 'tourist-ev',
        ...
        },
        {
            ...
            order: 3,
            content: {
                intro: 'From Bangkok to Phuket, Thailand is a top destination for Bangladeshi travellers. We handle tourist visa processing and visa-on-arrival assistance.',
                pricingTiers: [
                    {
                        id: 'tourist',
        ...
        },
        {
            ...
            order: 4,
            content: {
                intro: 'Applying for an Australian visa is a detailed process. We help Bangladeshi applicants prepare strong, complete applications for the Visitor visa (subclass 600).',
                pricingTiers: [
                    {
                        id: 'visitor-600',
        ...
        },
        {
            ...
            order: 5,
            content: {
                intro: 'The UK Standard Visitor Visa is popular with Bangladeshi travellers. We review documents, book appointments and support your submission.',
                pricingTiers: [
                    {
                        id: 'visitor-standard',
    ...
    // ===========================================================================
    const now = new Date();
    const inDays = (d, h = 8) => new Date(now.getFullYear(), now.getMonth(), now.getDate() + d, h, 0, 0);
    ...
    for (const f of flights) {
        ...
        if (existing)
            await prisma.flight.update({ where: { id: existing.id }, data: { pointsAwarded: 800 } });
        else {
    ...
    for (const tr of transports) {
        ...
        if (existing)
            await prisma.transport.update({ where: { id: existing.id }, data: { pointsAwarded: 300 } });
        else {
