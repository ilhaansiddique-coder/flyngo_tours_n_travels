...
export class CmsService {
  ...
  async listAllTestimonials(tenantId: string) {
    // Return {items, meta} (not a bare array) so the frontend api-client
    // normalizer exposes it under both .items and .data — a bare array has
    // neither, which made the admin list render empty.
  ...
  async listAllFaqs(tenantId: string) {
    // {items, meta} shape so the admin FAQ list isn't empty — see note on
    // listAllTestimonials.
