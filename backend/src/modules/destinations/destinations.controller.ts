...
export class DestinationsController {
  ...
  @Get('autocomplete')
  @Public()
  @ApiOperation({ summary: 'Country/destination autocomplete (flag + name)' })
  async autocomplete(
    @CurrentTenantId() tenantId: string,
    @Query('q') q?: string,
  ...
  @ApiOperation({ summary: 'Resolve a country/destination by name (auto-create if missing)' })
  async resolve(@CurrentTenantId() tenantId: string, @Body() body: any) {
    return this.destinationsService.resolve(tenantId, body?.name);
  }
