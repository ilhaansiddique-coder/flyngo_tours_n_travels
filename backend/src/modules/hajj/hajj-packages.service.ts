...
export class HajjPackagesService {
  ...
  async findById(id: string, tenantId: string) {
    const pkg = await this.prisma.hajjPackage.findFirst({ where: { id, tenantId, deletedAt: null } });
    if (!pkg) throw new NotFoundException('Hajj package not found');
    return pkg;
  }
  ...
  async create(tenantId: string, data: any) {
    ...
    return this.prisma.hajjPackage.create({
      data: {
        ...
        quadPrice: data.quadPrice ?? 0,
        triplePrice: data.triplePrice ?? 0,
        doublePrice: data.doublePrice ?? 0,
        balanceDueDate: data.balanceDueDate ? new Date(data.balanceDueDate) : null,
        requireMahramForFemales: data.requireMahramForFemales ?? true,
        passportValidityMonths: data.passportValidityMonths ?? 6,
  ...
  async update(id: string, tenantId: string, data: any) {
    ...
    return this.prisma.hajjPackage.update({
      ...
      data: {
        ...
        requireMahramForFemales: data.requireMahramForFemales,
