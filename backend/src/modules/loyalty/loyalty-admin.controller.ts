...
export class LoyaltyAdminController {
  ...
  @Post('backfill-signup-bonus')
  @ApiOperation({ summary: 'Grant the 100-point signup bonus to existing users that never got it' })
  async backfillSignupBonus(@CurrentTenantId() tenantId: string) {
    return this.loyaltyService.backfillSignupBonus(tenantId);
  }
  ...
  async adjust(
    ...
    return this.loyaltyService.adminAdjustPoints(tenantId, userId, body.points, body.reason, body.reference, adminId);
  }
}
