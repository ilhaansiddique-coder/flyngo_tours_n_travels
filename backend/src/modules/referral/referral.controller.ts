...
export class ReferralController {
  ...
  /**
   * Resolve a share message template for a given channel. Public so the
   * signup form and any other pre-auth surface can render the same copy
   * the dashboard will use after login. `code` is optional; when omitted
   * the resulting text just omits `{referralCode}` and `{shareLink}`.
   */
  @Get('share-message')
  @Public()
  @ApiOperation({ summary: 'Resolve a per-channel share message template' })
  async shareMessage(
    ...
    return this.referralService.resolveShareMessage(tenantId, channel, code);
  }
