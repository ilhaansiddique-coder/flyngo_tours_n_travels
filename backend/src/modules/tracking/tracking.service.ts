...
// =============================================================================
// Meta CAPI advanced-matching helpers (PII → SHA-256, Meta-compatible shapes)
// =============================================================================

/** SHA-256 hex digest. Meta expects lowercase hex for advanced matching fields. */
function sha256(value: string): string {
  return crypto.createHash('sha256').update(value.trim().toLowerCase()).digest('hex');
}

/** Normalize an email — trim + lowercase (Meta normalization for `em`). */
...
/** Normalize a phone to E.164-ready digits (7-15 digits; Meta normalization for `ph`). */
...
/** Split a full name into first/last for Meta `fn`/`ln` (both hashed). */
function splitFullName(fullName?: string | null): { fn?: string; ln?: string } {
  const parts = (fullName || '').trim().split(/\s+/).filter(Boolean);
  const fn = parts.shift();
  if (!fn) return {};
  return { fn, ln: parts.join(' ') || undefined };
}

/**
 * Build `user_data` for the Meta CAPI request.
 * Advanced matching fields (em/ph/fn/ln) are SHA-256 hashed. Fingerprint
 * cookies (fbp/fbc) and external_id pass through raw.
 */
function buildMetaUserData(evt: any): Record<string, any> {
  ...
  const phone = normalizePhone(evt.phone);
  if (phone) user_data.ph = sha256(phone);

  const { fn, ln } = splitFullName(evt.fullName);
  if (fn) user_data.fn = sha256(fn);
  if (ln) user_data.ln = sha256(ln);

  if (evt.externalId) user_data.external_id = String(evt.externalId);
  if (evt.fbp) user_data.fbp = String(evt.fbp);
  // Prefer the real browser fbc cookie; fall back to the value derived from fbclid.
  if (evt.fbc) user_data.fbc = String(evt.fbc);
  else if (evt.fbclid) user_data.fbc = `fb.1.${Math.floor(Date.now() / 1000)}.${evt.fbclid}`;
...
export class TrackingService {
  ...
  private async forwardToMetaCapi(evt: any, settings: any) {
    ...
    const data: any = {
      ...
      user_data: buildMetaUserData(evt),
      custom_data: {
    ...
    // Limited Data Use — only when the admin has switched compliance features on.
    // Unknown/no consent ⇒ LDU with Meta geolocation (country/state 0). Consent ⇒ no LDU.
    if (settings.metaLduEnabled && evt.consent !== 'all') {
