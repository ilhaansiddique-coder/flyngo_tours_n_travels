...
interface Settings {
  ...
  shareMessageTemplates?: Record<string, string> | null;
}
  ...
  const fetchOverview = useCallback(async () => {
    try {
      ...
      setSettingsForm({
        ...
        shareMessageTemplates: (settings as any).shareMessageTemplates ?? {},
      });
    } catch (err: any) {
  ...
  const saveSettings = async () => {
    ...
    try {
      const body: any = {
        ...
        shareMessageTemplates: settingsForm.shareMessageTemplates,
      };
    ...
    <div className="space-y-6">
      ...
      {tab === 'settings' && settingsForm && (
        <Card hover={false}>
            ...
            <div className="space-y-4">
              ...
              <FormField label="Type">
                <FormSelect
                  ...
                  options={[
                    { value: 'percentage', label: 'Percentage off first booking (e.g. 5%)' },
                    { value: 'fixed', label: `Fixed discount (e.g. 500 ${settingsForm.payoutCurrency})` },
                  ]}
                />
              </FormField>
              <FormField label="Value">
              ...
              <div className="p-3 rounded-xl bg-surface-container/60 border border-outline-variant text-xs text-on-surface-variant">
                <span className="font-bold text-on-surface">Share-link message:</span>{' '}
                {settingsForm.refereeRewardType === 'percentage'
                  ? `Friends get ${settingsForm.refereeRewardValue || 0}% off their first booking.`
                  : `Friends get ${settingsForm.refereeRewardValue || 0} ${settingsForm.payoutCurrency || 'BDT'} off their first booking.`}
              </div>
            </div>
          </div>
          ...
          {/* Per-channel share message templates — drive the WhatsApp / FB /
              X / Telegram / Email copy the dashboard and signup form use. */}
          <div className="mt-6 pt-6 border-t border-outline-variant space-y-4">
            <div>
              <h3 className="font-bold mb-1">Share message templates</h3>
              <p className="text-xs text-on-surface-variant">
                Per-channel copy for the referral share buttons. Use{' '}
                <code className="font-mono">{'{brand}'}</code>,{' '}
                <code className="font-mono">{'{refereeReward}'}</code>,{' '}
                <code className="font-mono">{'{referralCode}'}</code>,{' '}
                <code className="font-mono">{'{shareLink}'}</code>, and{' '}
                <code className="font-mono">{'{refereeName}'}</code> as placeholders. Leave blank to use the default.
              </p>
            </div>
            {([
              ['whatsapp', 'WhatsApp'],
              ...
              ['signup_banner', 'Signup banner (shown above the register form)'],
            ] as const).map(([key, label]) => (
              <FormField key={key} label={label}>
                <FormTextarea
                  value={(settingsForm.shareMessageTemplates || {})[key] || ''}
                  onChange={(v: string) =>
                    setSettingsForm({
                      ...settingsForm,
                      shareMessageTemplates: {
                        ...(settingsForm.shareMessageTemplates || {}),
                        [key]: v,
                      },
                    })
                  }
                  rows={key === 'email_body' ? 4 : 2}
