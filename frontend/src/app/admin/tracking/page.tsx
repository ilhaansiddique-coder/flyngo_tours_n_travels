  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setSettingsForm({
        ...
        requireMarketingConsent: (s as TrackingSettings).requireMarketingConsent ?? false,
        metaLduEnabled: (s as TrackingSettings).metaLduEnabled ?? false,
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  className="rounded accent-accent"
                  checked={settingsForm.requireMarketingConsent}
                  onChange={(e) => setSettingsForm({ ...settingsForm, requireMarketingConsent: e.target.checked })}
                />
                <span className="text-sm">
                  Require marketing consent before loading pixels <span className="text-xs text-on-surface-variant">(turn on before EU/UK launch)</span>
                </span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  className="rounded accent-accent"
                  checked={settingsForm.metaLduEnabled}
                  onChange={(e) => setSettingsForm({ ...settingsForm, metaLduEnabled: e.target.checked })}
                />
                <span className="text-sm">
                  Send Meta Limited Data Use (LDU) for non-consented events <span className="text-xs text-on-surface-variant">(EEA/UK/US compliance)</span>
                </span>
              </label>
