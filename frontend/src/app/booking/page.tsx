      ...
      <div className="min-h-screen bg-surface flex items-center justify-center pt-20">
        <div className="max-w-md w-full px-4">
          <div className="glass text-center p-12 rounded-2xl border-outline-variant">
            ...
            <h2 className="font-display text-2xl font-bold text-on-surface mb-2">Booking Confirmed!</h2>
            <p className="text-on-surface-variant mb-4">Your booking has been submitted successfully.</p>
            ...
            <div key={step.number} className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
                  currentStep >= step.number ? 'bg-[#00eefc] text-surface' : 'bg-surface-container text-on-surface-variant'
                }`}>
                  {currentStep > step.number ? <Check className="w-4 h-4" /> : step.number}
                </div>
                <span className={`text-sm font-medium ${currentStep >= step.number ? 'text-on-surface' : 'text-on-surface-variant'}`}>
                  {step.label}
                </span>
              </div>
              {i < steps.length - 1 && (
                <div className={`w-12 h-0.5 ${currentStep > step.number ? 'bg-[#00eefc]' : 'bg-surface-container'}`} />
              )}
            </div>
          ))}
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between py-3 border-b border-outline-variant text-on-surface-variant">
                  ...
                  <span className="font-medium text-on-surface">{formData.firstName} {formData.lastName}</span>
                </div>
                <div className="flex justify-between py-3 border-b border-outline-variant text-on-surface-variant">
                  <span>Email</span>
