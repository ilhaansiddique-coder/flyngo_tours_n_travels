...
export default function HajjPackageDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await getHajjPackages({ limit: '100' });
        const all: HajjPackage[] = ((res as any)?.items ?? []) as HajjPackage[];
        setPkg(all.find((p) => p.slug === slug) ?? null);
      } finally {
        setLoading(false);
      }
    })();
  }, [slug, getHajjPackages]);

  const book = () => {
    if (!pkg) return;
    setSelectedItem(pkg.id);
    router.push(`/booking?type=hajj&id=${pkg.id}`);
  };
    ...
    <main className="min-h-screen pt-28 pb-16 px-4 sm:px-6 lg:px-16 max-w-[1200px] mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8">
        <div>
          ...
          {(Array.isArray(pkg.departureCities) && pkg.departureCities.length > 0) || pkg.departureDate ? (
            <section className="mb-8">
              <h2 className="text-xl font-display font-semibold mb-4">Departure</h2>
              <div className="space-y-2 text-on-surface">
                {Array.isArray(pkg.departureCities) && pkg.departureCities.length > 0 && (
                  <div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-muted" /> {pkg.departureCities.join(', ')}</div>
                )}
                {pkg.departureDate && (
                  <div className="flex items-center gap-2"><CalendarDays className="w-4 h-4 text-muted" /> {formatDate(pkg.departureDate)}{pkg.returnDate ? ` → ${formatDate(pkg.returnDate)}` : ''}</div>
                )}
              </div>
            </section>
          ) : null}
          ...
          <section>
            ...
            <ol className="space-y-3">
              {[
                'Reserve your seat with a booking — earn loyalty points on completion.',
                'Our team confirms your package, documents and visa requirements.',
                'Complete payment in instalments up to the balance due date.',
                'Travel with a guided group; we handle hotels, transport and ziyarat.',
              ].map((step, i) => (
        ...
        <aside className="lg:sticky lg:top-28 self-start">
          <div className="rounded-2xl border glass p-6" style={{ borderColor: 'var(--color-outline-variant)' }}>
            ...
            <div className="mt-4 pt-4 border-t space-y-3 text-sm" style={{ borderColor: 'var(--color-outline-variant)' }}>
              ...
              {seatsLeft != null && (
                <div className="flex items-center gap-2"><Users className="w-4 h-4 text-muted" /> {seatsLeft > 0 ? `${seatsLeft} seats left` : 'Sold out'}</div>
              )}
            </div>
            ...
            <Link href="/hajj" className="mt-2 block text-center text-xs text-muted hover:underline">
              Or pre-register your interest
            </Link>
          </div>
        </aside>
      </div>

      <ReviewsSection itemType="hajj" itemId={pkg.id} />
    </main>
  );
}
