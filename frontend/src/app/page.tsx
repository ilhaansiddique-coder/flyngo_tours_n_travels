    ...
    <main>
      ...
      <section className="mt-32 px-16 max-w-[1440px] mx-auto mb-32">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div>
            <h2 className="font-display text-[48px] leading-tight font-semibold text-white mb-4">Curated Experiences</h2>
          ...
          <div className="flex gap-4">
            <button className="glass p-3 rounded-full hover:bg-white/10 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="group relative md:col-span-2 h-[500px] rounded-xl overflow-hidden glass velocity-glow cursor-pointer">
            ...
            <div className="absolute bottom-0 left-0 p-10">
              ...
              <h3 className="font-display text-[32px] font-semibold text-white mb-4">The Santorini Sky Loft</h3>
              ...
              <button className="flex items-center gap-2 text-sm font-semibold text-white group-hover:text-[#00eefc] transition-colors">
          ...
          <div className="group relative h-[500px] rounded-xl overflow-hidden glass velocity-glow cursor-pointer">
            ...
            <div className="absolute bottom-0 left-0 p-8">
              ...
              <h3 className="font-display text-[32px] font-semibold text-white mb-4">Priority Skies</h3>
      ...
      <section className="bg-surface-container py-24 mb-32 relative overflow-hidden">
        ...
        <div className="px-16 max-w-[1440px] mx-auto grid grid-cols-2 lg:grid-cols-4 gap-12 text-center">
          {stats.map((stat) => (
            <div key={stat.label}>
              <span className={`block font-display text-[48px] leading-tight font-semibold mb-2 ${stat.accent ? 'text-[#00eefc]' : 'text-white'}`}>
                {stat.value}
              </span>
              <span className="text-sm tracking-[0.1em] uppercase text-white/60 font-semibold">{stat.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-16 max-w-[1440px] mx-auto mb-32">
        <div className="flex items-center gap-4 mb-12">
          ...
          <h2 className="text-sm tracking-[0.2em] uppercase text-white/60 font-semibold whitespace-nowrap">Popular High Velocity Routes</h2>
          <div className="h-[1px] flex-1 bg-white/10" />
        </div>
          ...
          {routes.map((route) => (
            ...
            >
              <div className="flex justify-between items-start mb-8">
                <div>
                  <span className="font-display text-[32px] font-semibold text-white block">{route.from}</span>
                  <span className="text-xs text-white/60">{route.fromCity}</span>
                </div>
                <PlaneTakeoff className="text-[#00eefc] w-6 h-6 mt-2" />
                <div className="text-right">
                  <span className="font-display text-[32px] font-semibold text-white block">{route.to}</span>
                  <span className="text-xs text-white/60">{route.toCity}</span>
                </div>
              </div>
              <div className="flex justify-between items-center text-xs text-white/60 mb-6">
                <span>{route.duration}</span>
                <span className="text-white font-bold">From {route.price}</span>
              </div>
      ...
      <section className="px-16 max-w-[1440px] mx-auto mb-32">
        <div className="glass-deep p-16 rounded-2xl relative overflow-hidden flex flex-col items-center text-center">
          ...
          <h2 className="font-display text-[48px] leading-tight font-semibold text-white mb-6">Join the Velocity Elite</h2>
          <p className="text-lg leading-relaxed text-white/60 mb-10 max-w-xl">
          ...
          <form className="flex flex-col md:flex-row gap-4 w-full max-w-lg" onSubmit={(e) => e.preventDefault()}>
            <input
              className="flex-1 glass bg-white/5 border-white/10 px-6 py-4 rounded-full outline-none focus:border-[#00eefc] transition-all text-white placeholder:text-white/40"
              placeholder="Enter your email address"
              type="email"
            />
            <button className="bg-white text-[#031427] px-10 py-4 rounded-full text-sm font-bold tracking-wider transition-all hover:bg-[#00eefc] hover:text-[#00686f] whitespace-nowrap">
              Join Now
            </button>
          </form>
        </div>
